...
export const patchDataWithRoutes: IPatchDataWithRoutes = (
  ...
  routes.forEach(route => {
    ...
    const webpackChunkName = normalizeEntry(
    ...
    const chunks = flatten(
      chunkGroupData
        .filter(
          group => group.name === webpackChunkName || group.name === 'umi'
        )
 ...
 * @param dynamicMaps
 */
export const getChunkAssetsMaps = (
  dynamicMaps: IDynamicMap
): IDynamicMapAssets =>
